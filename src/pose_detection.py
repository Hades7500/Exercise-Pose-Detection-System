...
LANDMARKS = [11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28, 29, 30]
...
class PoseResultStore:
    """Thread-safe container for the latest PoseLandmarker result."""
    def __init__(self):
...
pose_result_store = PoseResultStore()

def _result_callback(result, output_image, timestamp_ms):
    """
    Called by MediaPipe on a background thread each time detection completes.
    Just saves the result — never blocks.
    """
    pose_result_store.update(result)
